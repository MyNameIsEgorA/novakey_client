...
import { Search, Home, MapPin, MessageSquare, User, Plus, Calendar, Users, Building2, Menu, X, Bell, Settings, List } from "lucide-react";
...
type AuthState = 'selection' | 'buyer-auth' | 'developer-auth' | 'authenticated';
type UserType = 'buyer' | 'developer' | null;
type Screen = 'home' | 'map' | 'list' | 'detail' | 'chat' | 'profile' | 'ar-viewer' | 'add-property' | 'crm' | 'calendar';
...
export default function App() {
  ...
  const navigationItems = userType === 'buyer' ? [
    { id: 'home', icon: Home, label: 'Главная', screen: 'home' as Screen },
    { id: 'map', icon: MapPin, label: 'Карта', screen: 'map' as Screen },
    { id: 'list', icon: Search, label: 'Поиск', screen: 'list' as Screen },
    { id: 'chat', icon: MessageSquare, label: 'Чаты', screen: 'chat' as Screen },
    { id: 'profile', icon: User, label: 'Профиль', screen: 'profile' as Screen },
  ] : [
    { id: 'home', icon: Home, label: 'Главная', screen: 'home' as Screen },
    { id: 'map', icon: Building2, label: 'Карта', screen: 'map' as Screen },
    { id: 'list', icon: List, label: 'Мои объекты', screen: 'list' as Screen },
    { id: 'crm', icon: Users, label: 'CRM', screen: 'crm' as Screen },
    { id: 'calendar', icon: Calendar, label: 'Календарь', screen: 'calendar' as Screen },
    { id: 'profile', icon: User, label: 'Профиль', screen: 'profile' as Screen },
  ];
  ...
  if (authState !== 'authenticated') {
    switch (authState) {
      ...
      case 'developer-auth':
        return <DeveloperAuth onBack={handleBack} onSuccess={handleAuthSuccess} />;
    }
  }

  // Fullscreen components (AR Viewer)
  if (currentScreen === 'ar-viewer') {
    return <ARViewer propertyId={selectedPropertyId!} onClose={() => setCurrentScreen('detail')} />;
  }
      ...
      <div className="lg:hidden">
        <div className="max-w-md mx-auto bg-white min-h-screen">
          ...
          {currentScreen !== 'home' ? (
            ...
            <div>
              {currentScreen === 'map' && <PropertyMap onPropertySelect={handlePropertySelect} onBack={() => setCurrentScreen('home')} />}
              {currentScreen === 'list' && <PropertyList onPropertySelect={handlePropertySelect} onBack={() => setCurrentScreen('home')} userType={userType} />}
              {currentScreen === 'detail' && <PropertyDetail propertyId={selectedPropertyId!} onBack={() => setCurrentScreen('home')} onStartChat={() => setCurrentScreen('chat')} onStartARViewer={handleStartARViewer} userType={userType} />}
              {currentScreen === 'chat' && <Chat propertyId={selectedPropertyId} onBack={() => setCurrentScreen('detail')} />}
              {currentScreen === 'profile' && <Profile userType={userType!} onBack={() => setCurrentScreen('home')} onLogout={handleLogout} />}
              {currentScreen === 'add-property' && <AddProperty onBack={() => setCurrentScreen('home')} onSave={handlePropertySave} />}
              {currentScreen === 'crm' && <DeveloperCRM onBack={() => setCurrentScreen('home')} onStartChat={(userId) => setCurrentScreen('chat')} />}
              {currentScreen === 'calendar' && <ViewingCalendar onBack={() => setCurrentScreen('home')} />}
            </div>
          ) : (
            // Mobile Home Screen
            <div>
              ...
              <div className="px-6 pt-12 pb-4 bg-white">
                <div className="flex items-center justify-between mb-6">
                  <div>
                    <p className="text-gray-500 text-sm mb-1">
                      {userType === 'buyer' ? 'Доброе утро,' : 'Добро пожаловать,'}
                    </p>
                    <h1 className="text-black">
                      {userType === 'buyer' ? 'Анна' : 'Александр Иванов'}
                    </h1>
                    {userType === 'developer' && (
                      <p className="text-sm text-emerald-600">ООО "СтройИнвест" • Рейтинг 4.8 ⭐</p>
                    )}
                  </div>
                  <button 
                ...
                <div className="relative">
                  <input
                    type="text"
                    placeholder={userType === 'buyer' ? "Поиск квартир, районов..." : "Поиск по объектам, клиентам..."}
              ...
              {userType === 'buyer' ? (
                ...
                <div className="px-6 pb-20">
                  <div className="bg-gradient-to-r from-emerald-500 to-emerald-600 text-white rounded-xl p-6 mb-6">
                    ...
                    <p className="text-emerald-100 text-sm">Управление объектами недвижимости</p>
                  </div>
                    ...
                    <div className="space-y-3">
                      <div className="bg-white rounded-xl p-4 shadow-sm border">
                        <div className="flex items-center">
                          ...
                          <div className="flex-1">
                            <p className="text-black text-sm">Новая заявка от Анны Смирновой</p>
                            <p className="text-gray-500 text-xs">ЖК "Северная звезда" • 2 часа назад</p>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="bg-white rounded-xl p-4 shadow-sm border">
                        <div className="flex items-center">
                          ...
                          <div className="flex-1">
                            <p className="text-black text-sm">Подтвержден показ с Михаилом Петровым</p>
                            <p className="text-gray-500 text-xs">Завтра в 16:30 • VR-просмотр</p>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="bg-white rounded-xl p-4 shadow-sm border">
                        <div className="flex items-center">
                          ...
                          <div className="flex-1">
                            <p className="text-black text-sm">Объект прошел проверку ЕГРН</p>
                            <p className="text-gray-500 text-xs">ЖК "Парковый" • 1 день назад</p>
                          </div>
                        </div>
                      </div>
                    </div>
        ...
        <div className="w-64 bg-white border-r border-gray-200 flex flex-col">
          ...
          <div className="p-6 border-b border-gray-200">
            <div className="flex items-center">
              ...
              <div>
                <p className="text-gray-500 text-sm">{userType === 'buyer' ? 'Покупатель' : 'Застройщик'}</p>
              </div>
            </div>
          </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                <h1 className="text-2xl text-black">
                  ...
                  {currentScreen === 'map' && (userType === 'buyer' ? 'Карта объектов' : 'Карта объектов')}
                  {currentScreen === 'list' && (userType === 'buyer' ? 'Поиск недвижимости' : 'Мои объекты')}
                ...
                <p className="text-gray-500">
                  {currentScreen === 'home' && (userType === 'buyer' ? 'Найдите идеальную недвижимость' : 'Управляйте вашими объектами')}
                  {currentScreen === 'map' && 'Интерактивная карта с объектами'}
                  {currentScreen === 'list' && (userType === 'buyer' ? 'Список всех доступных объектов' : 'Управление вашими объектами')}
                  {currentScreen === 'crm' && 'Управление клиентами и заявками'}
                  {currentScreen === 'calendar' && 'Планирование показов'}
                  {currentScreen === 'chat' && 'Общение с застройщиками и покупателями'}
                  {currentScreen === 'profile' && 'Управление профилем и настройками'}
                  {currentScreen === 'add-property' && 'Добавление нового объекта недвижимости'}
                </p>
              </div>
                      ...
                      <div className="mb-8">
                        ...
                        <div className="grid grid-cols-3 gap-6">
                          ...
                          >
                            ...
                            <p className="text-gray-500 text-sm">Поиск на интерактивной карте</p>
                          </button>
                          <button 
                            onClick={() => setCurrentScreen('list')}
                          ...
                          >
                            ...
                            <p className="text-gray-500 text-sm">Все объявления с фильтрами</p>
                          </button>
                          <button 
                          ...
                          >
                            ...
                            <p className="text-gray-500 text-sm">Общение с застройщиками</p>
                          </button>
                        </div>
                      </div>
                        ...
                        <div className="bg-white rounded-xl p-6 shadow-sm border">
                          ...
                          <div className="space-y-4">
                            <div className="flex items-start">
                              ...
                              <div>
                                <p className="text-sm text-black">Просмотрели ЖК "Центральный"</p>
                            ...
                            <div className="flex items-start">
                              ...
                              <div>
                                <p className="text-sm text-black">Связались с застройщиком</p>
                  ...
                  <div className="grid grid-cols-12 gap-8">
                    ...
                    <div className="col-span-8">
                      <div className="bg-gradient-to-r from-emerald-500 to-emerald-600 text-white rounded-xl p-8 mb-8">
                        ...
                        <p className="text-emerald-100">Управление объектами недвижимости и клиентами</p>
                      </div>
                      ...
                      <div>
                        ...
                        <div className="grid grid-cols-4 gap-6">
                          <div className="bg-white rounded-xl p-6 shadow-sm border text-center">
                            <h3 className="text-black mb-2">Активные объекты</h3>
                            <div className="text-3xl text-emerald-600 mb-2">12</div>
                            <p className="text-gray-400 text-sm">ЖК в продаже</p>
                          </div>
                          ...
                          <div className="bg-white rounded-xl p-6 shadow-sm border text-center">
                            <h3 className="text-black mb-2">Показы на неделю</h3>
                            <div className="text-3xl text-purple-600 mb-2">15</div>
                            <p className="text-gray-400 text-sm">запланировано</p>
                          </div>
                          
                          <div className="bg-white rounded-xl p-6 shadow-sm border text-center">
                            <h3 className="text-black mb-2">Продажи за месяц</h3>
                            <div className="text-3xl text-green-600 mb-2">24</div>
                    ...
                    <div className="col-span-4">
                      <div className="space-y-6">
                        {/* Recent Activity */}
                        <div className="bg-white rounded-xl p-6 shadow-sm border">
                          <h3 className="text-black mb-4">Последняя активность</h3>
                          <div className="space-y-4">
                            <div className="flex items-center">
