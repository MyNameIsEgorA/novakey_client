...
import { ArrowLeft, Mail, Lock, Eye, EyeOff, Building2, Shield, Users } from "lucide-react";
...
export function DeveloperAuth({ onBack, onSuccess }: DeveloperAuthProps) {
  ...
  const handleRequestAccess = async (e: React.FormEvent) => {
    ...
    // Simulate API call
    setTimeout(() => {
      alert('Заявка на получение доступа отправлена! Наш менеджер свяжется с вами в течение 24 часов.');
      setIsLoading(false);
          ...
          <div className="px-6 pt-12 pb-8">
            ...
            <div className="text-center mb-8">
              ...
              <p className="text-gray-600">
                {isRequestAccess ? 'Заполните заявку на получение доступа' : 'Войдите в панель застройщика'}
              </p>
            </div>
          </div>
            ...
            <form onSubmit={isRequestAccess ? handleRequestAccess : handleSubmit} className="space-y-6">
              ...
              {isRequestAccess && (
                <>
                  ...
                  <div>
                    <label className="block text-gray-700 mb-2">Краткое описание деятельности</label>
                    <textarea
                ...
                <div className="flex items-start">
                  ...
                  <div>
                    <h4 className="text-emerald-900 mb-1">
                      {isRequestAccess ? 'Проверка заявки' : 'Доступ только для партнеров'}
                    </h4>
              ...
              >
                {isLoading ? (
                  ...
                  isRequestAccess ? 'Отправить заявку' : 'Войти в систему'
                )}
              </button>
            </form>
                  ...
                  >
                    ...
                    <div className="h-full flex flex-col justify-between">
                      <div className="text-sm">
                        Детальная аналитика<br />
                        и отчетность
                      </div>
                      <div>
                        <div className="text-2xl mb-2">95%</div>
                        <div className="text-sm opacity-80">Конверсия показов</div>
                      </div>
                    </div>
                  </div>
              ...
              <form onSubmit={isRequestAccess ? handleRequestAccess : handleSubmit} className="space-y-6">
                  ...
                  <div className="flex items-start">
                    ...
                    <div>
                      <h4 className="text-emerald-900 mb-2">
                        {isRequestAccess ? 'Быстрое рассмотрение заявки' : 'Доступ только для партнеров'}
                      </h4>
                ...
                >
                  {isLoading ? (
                    <>
                      <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin mr-2"></div>
                      {isRequestAccess ? 'Отправка заявки...' : 'Выполняется вход...'}
                    </>
                  ) : (
                    isRequestAccess ? 'Отправить заявку на доступ' : 'Войти в систему'
                  )}
                </button>
              </form>
