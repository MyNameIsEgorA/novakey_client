...
import { ArrowLeft, MapPin, Calendar, Maximize, Heart, Share2, Phone, MessageSquare, Camera, Users, Edit, BarChart3, Star, Eye, Clock, Home } from "lucide-react";
...
export function PropertyDetail({ propertyId, onBack, onStartChat, onStartARViewer, userType }: PropertyDetailProps) {
  ...
  const property = {
    ...
    description: 'Современный жилой комплекс в развивающемся районе с отличной транспортной доступностью. Квартира с улучшенной планировкой, большими окнами и видом на парк.',
    images: [
          ...
          <div className="p-6">
            ...
            <div className="mb-6">
              <div className="flex items-center justify-between mb-2">
                <h1 className="text-2xl text-black">{property.name}</h1>
              ...
              {userType === 'developer' && (
                <div className="grid grid-cols-2 gap-4 mb-6 p-4 bg-gray-50 rounded-xl">
                  <div className="text-center">
                    <div className="text-xl text-emerald-600">{property.soldApartments}/{property.totalApartments}</div>
                    <div className="text-xs text-gray-500">Продано</div>
                  </div>
                  <div className="text-center">
                    <div className="text-xl text-blue-600">{property.viewsCount}</div>
                    <div className="text-xs text-gray-500">Просмотров</div>
                  </div>
                  <div className="text-center">
                    <div className="text-xl text-purple-600">{property.inquiriesCount}</div>
                    <div className="text-xs text-gray-500">Заявок</div>
                  </div>
                  <div className="text-center">
                    <div className="text-xl text-orange-600">{property.averageViewTime}</div>
              ...
              <div className="grid grid-cols-3 gap-4 p-4 bg-gray-50 rounded-xl mb-6">
                ...
                <div className="text-center">
                  ...
                  <div className="text-black">{property.floor}/{property.maxFloor}</div>
            ...
            <div className="mb-6">
              ...
              <div className="space-y-2">
                <div className="flex items-center">
                  <span className={`w-2 h-2 rounded-full mr-3 ${property.nearTransport ? 'bg-green-500' : 'bg-gray-300'}`}></span>
                  <span className="text-gray-700">Транспорт рядом</span>
                </div>
                <div className="flex items-center">
                  <span className={`w-2 h-2 rounded-full mr-3 ${property.nearSchool ? 'bg-green-500' : 'bg-gray-300'}`}></span>
                  <span className="text-gray-700">Школа рядом</span>
                </div>
                <div className="flex items-center">
                  <span className={`w-2 h-2 rounded-full mr-3 ${property.nearShops ? 'bg-green-500' : 'bg-gray-300'}`}></span>
                    ...
                    {property.images.map((image, index) => (
                      <button
                        ...
                        className={`flex-shrink-0 w-20 h-16 rounded-lg overflow-hidden border-3 transition-all ${
                          activeImageIndex === index ? 'border-white shadow-lg' : 'border-white/50'
                        }`}
                      >
                        <ImageWithFallback
                          src={image}
          ...
          <div className="col-span-5 bg-white p-8 overflow-y-auto">
            ...
            <div className="mb-8">
              ...
              <div className="flex items-center justify-between mb-6">
                <div>
                  <p className="text-4xl text-blue-600 mb-1">{property.price}</p>
                  <p className="text-gray-500">{property.pricePerSqm.toLocaleString()} ₽/м²</p>
                </div>
                {userType === 'buyer' && (
                  <div className="flex items-center">
                    <Star className="w-5 h-5 text-yellow-500 mr-2" />
                    <span className="text-xl text-gray-700">{property.rating}</span>
                    <span className="text-gray-500 ml-2">({property.reviews} отзывов)</span>
                  </div>
                )}
              </div>
              ...
              {userType === 'developer' && (
                <div className="grid grid-cols-2 gap-4 mb-8 p-6 bg-gray-50 rounded-xl">
                  <div className="text-center">
                    <div className="text-2xl text-emerald-600 mb-1">{property.soldApartments}/{property.totalApartments}</div>
                    <div className="text-sm text-gray-500">Продано квартир</div>
                  </div>
                  <div className="text-center">
                    <div className="text-2xl text-blue-600 mb-1">{property.viewsCount}</div>
                    <div className="text-sm text-gray-500">Просмотров</div>
                  </div>
                  <div className="text-center">
                    <div className="text-2xl text-purple-600 mb-1">{property.inquiriesCount}</div>
                    <div className="text-sm text-gray-500">Заявок</div>
                  </div>
                  <div className="text-center">
                    <div className="text-2xl text-orange-600 mb-1">{property.averageViewTime}</div>
              ...
              <div className="grid grid-cols-3 gap-6 p-6 bg-gray-50 rounded-xl mb-8">
                <div className="text-center">
                  ...
                  <div className="text-xl text-black mb-1">{property.rooms}</div>
                  <div className="text-sm text-gray-500">Планировка</div>
                </div>
                <div className="text-center">
                  <Maximize className="w-8 h-8 text-gray-600 mx-auto mb-2" />
                  <div className="text-xl text-black mb-1">{property.area} м²</div>
                  <div className="text-sm text-gray-500">Площадь</div>
                </div>
                <div className="text-center">
                  <Calendar className="w-8 h-8 text-gray-600 mx-auto mb-2" />
                  <div className="text-xl text-black mb-1">{property.floor}/{property.maxFloor}</div>
            ...
            <div className="mb-8">
              ...
              <div className="space-y-3">
                <div className="flex items-center">
                  <span className={`w-3 h-3 rounded-full mr-4 ${property.nearTransport ? 'bg-green-500' : 'bg-gray-300'}`}></span>
                  <span className="text-gray-700">Транспорт рядом</span>
                </div>
                <div className="flex items-center">
                  <span className={`w-3 h-3 rounded-full mr-4 ${property.nearSchool ? 'bg-green-500' : 'bg-gray-300'}`}></span>
                  <span className="text-gray-700">Школа рядом</span>
                </div>
                <div className="flex items-center">
                  <span className={`w-3 h-3 rounded-full mr-4 ${property.nearShops ? 'bg-green-500' : 'bg-gray-300'}`}></span>
                  <span className="text-gray-700">Магазины рядом</span>
                </div>
              </div>
            </div>
